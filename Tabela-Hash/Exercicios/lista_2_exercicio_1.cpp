...
int buscar(HashTabela tabela, int n) {
    ...
    int pos2 = funcao_hashing2(n);

    while (i < Tsize && 
           tabela.tHash[(pos + i*pos2) % Tsize] != -1 && 
           tabela.tHash[(pos + i*pos2) % Tsize] != n) {
        i = i + 1; 
    }
    if (tabela.tHash[(pos + i*pos2) % Tsize] == n) {
        return (pos + i*pos2) % Tsize;
    } else {
