...
void Hash::retrieveItem(Aluno& aluno, bool& found) {
    ...
    if (aluno.getRa() != aux.getRa()) {
        found = false; // Item não encontrado
    } else {
        found = true; // Item encontrado
        aluno = aux; // Copia o aluno encontrado para o parâmetro passado
    }
}
...
void Hash::deleteItem(Aluno aluno) {
    int location = getHash(aluno);
    structure[location] = Aluno(); // Substitui o item por um objeto Aluno vazio
    length--;
}
